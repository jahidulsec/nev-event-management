...
import { createEventStatus } from "../actions/events-status";
import {
...
export default function EventStatusUpdateForm({
  ...
  const form = useForm<CreateEventStatusPayloadType>({
    resolver: zodResolver(createEventStatusPayloadSchema),
    defaultValues: {
      event_id: event.id,
      employee_id: authUser.employeeId,
      sap_area_code: event.sap_area_code,
      user_role: role as CreateEventStatusPayloadType["user_role"],
    },
  });

  const onSubmit = async (payload: CreateEventStatusPayloadType) => {
    const res = await createEventStatus({
      ...payload,
      remarks: `${eventType}: ${payload.remarks}`,
    });
  ...
  return (
    <Form className="w-full max-w-2xl" onSubmit={form.handleSubmit(onSubmit)}>
      ...
      <FormButton
        ...
        disabled={
          (role === "ec" &&
            !event.event_consultants.every(
              (i) => i.event_consultant_approvals?.ec_id,
            )) ||
          (role === firstApproverRole &&
            !event.event_consultants.every(
              (i) => i.event_consultant_approvals?.first_approver_id,
            ))
        }
      >
        Submit
