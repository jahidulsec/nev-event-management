...
import {
  ...
  CreateEventPayloadInputType,
  CreateEventPayloadType,
} from "../schemas/events";
import {
  EventTypeMultiProps,
  getEventTypes,
} from "@/features/event-type/libs/event-type";
import { EventBudgetSection } from "./event-budget-section";
import ConsultantSection from "@/features/event-consultants/components/consultant-section";
...
export default function EventForm({
  ...
  prevData?: Partial<CreateEventPayloadType & { id: string }>;
  ...
  >({
    ...
    defaultValues: {
      ...
      sap_area_code: prevData?.sap_area_code,
      ...
      eventAttachment: prevData?.eventAttachment,
      eventConsultant: prevData?.eventConsultant,
      eventBudget: prevData?.eventBudget,
    },
  });
  ...
  const isWithoutSpeaker = eventType?.toLowerCase().includes("without speaker");

  async function onSubmit(data: CreateEventPayloadType) {
    if (!prevData) {
